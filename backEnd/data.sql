-- ============================================================
-- 花卉识别系统数据库创建脚本（适配MySQL 5.X）
-- 数据库: flower_recognition
-- 版本: 1.0
-- 适配版本: MySQL 5.5.3+ (支持utf8mb4)
-- 创建日期: 2026-01-23
-- ============================================================

-- 1. 创建数据库（MySQL 5.X建议显式指定字符集和排序规则）
CREATE DATABASE IF NOT EXISTS `flower_recognition` 
...
CREATE TABLE IF NOT EXISTS `recognition_history` (
  ...
  -- 关键修改1：MySQL 5.X不支持JSON，改为TEXT存储JSON字符串
  `top_k_results` TEXT DEFAULT NULL COMMENT 'Top K预测结果(JSON格式字符串)',
